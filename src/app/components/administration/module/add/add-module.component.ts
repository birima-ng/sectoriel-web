...
add(){
    ...
    this.moduleService.createModule(this.addForm.value).subscribe(
      data => {
if(data){
...
this.toastr.success('Module ajouté avec succès!', 'STOCK-PRIX');
}else {
 this.spinner.hide();
...
edit(){
    ...
    this.moduleService.updateModule(this.addForm.value).subscribe(
      data => {
if(data){
console.log("################################ Data ", data);
 this.spinner.hide();
...
//fermer le popup
this.activeModal.close('Data updated');
this.toastr.success('Module modifié avec succès!', 'STOCK-PRIX');
}else {
 this.toastr.error('Le code ou le nom  existe déjà!', 'STOCK-PRIX');
}
      },
