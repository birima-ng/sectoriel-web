...
@Component({
  selector: 'app-add-etat-pneus',
  templateUrl: './add-etat-pneus.component.html',
  styleUrls: ['./add-etat-pneus.component.scss']
})

export class AddEtatPneusComponent {
invalidLogin = false;
submitted=false;
lng="en";
etatPneus: EtatPneus;
...
add(){
    ...
    this.etatPneusService.createEtatPneus(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
...
this.toastr.success("Etat pneus ajouté avec succès!", 'STOCK-PRIX');
}else {
...
edit(){
    ...
    this.etatPneusService.updateEtatPneus(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
...
//fermer le popup
this.activeModal.close('Data updated');
this.toastr.success("Etat pneus modifié avec succès!", 'STOCK-PRIX');
}else {
 this.toastr.error('Le code ou le libellé  existe déjà!', 'STOCK-PRIX');
}
      },
