...
@Component({
  selector: 'app-add-decoupage1',
  templateUrl: './add-decoupage1.component.html',
  styleUrls: ['./add-decoupage1.component.scss']
})

export class AddDecoupage1Component {
invalidLogin = false;
submitted=false;
lng="en";
...
add(){
    ...
    this.decoupage1Service.createDecoupage1(this.addForm.value).subscribe(
      data => {
 if(data){
 this.spinner.hide();
...
edit(){
    ...
    this.decoupage1Service.updateDecoupage1(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
