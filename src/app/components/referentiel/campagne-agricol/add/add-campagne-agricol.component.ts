...
@Component({
  selector: 'app-add-campagne-agricol',
  templateUrl: './add-campagne-agricol.component.html',
  styleUrls: ['./add-campagne-agricol.component.scss']
})

export class AddCampagneAgricolComponent {
invalidLogin = false;
submitted=false;
lng="en";
campagneagricol: CampagneAgricol;
...
add(){
    ...
    this.campagneagricolService.createCampagneAgricol(this.addForm.value).subscribe(
      data => {
 if(data){
 this.spinner.hide();
...
edit(){
    ...
    this.campagneagricolService.updateCampagneAgricol(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
