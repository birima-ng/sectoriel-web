...
export class AddSpeculationComponent {
invalidLogin = false;
submitted=false;
lng="en";
...
add(){
    ...
    this.speculationService.createSpeculation(this.addForm.value).subscribe(
      data => {
 if(data){
 this.spinner.hide();
...
edit(){
    ...
    this.speculationService.updateSpeculation(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
