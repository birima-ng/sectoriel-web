...
@Component({
  selector: 'app-add-decoupage2',
  templateUrl: './add-decoupage2.component.html',
  styleUrls: ['./add-decoupage2.component.scss']
})

export class AddDecoupage2Component {
invalidLogin = false;
submitted=false;
lng="en";
...
add(){
    ...
    this.decoupage2Service.createDecoupage2(this.addForm.value).subscribe(
      data => {
 if(data){
 this.spinner.hide();
...
edit(){
    ...
    this.decoupage2Service.updateDecoupage2(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
