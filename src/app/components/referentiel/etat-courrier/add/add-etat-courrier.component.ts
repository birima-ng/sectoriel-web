...
@Component({
  selector: 'app-add-etat-courrier',
  templateUrl: './add-etat-courrier.component.html',
  styleUrls: ['./add-etat-courrier.component.scss']
})

export class AddEtatCourrierComponent {
invalidLogin = false;
submitted=false;
lng="en";
...
add(){
    ...
    this.etatCourrierService.createEtatCourrier(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
...
this.toastr.success("Etat général ajouté avec succès!", 'STOCK-PRIX');
}else {
...
edit(){
    ...
    this.etatCourrierService.updateEtatCourrier(this.addForm.value).subscribe(
      data => {
...
//fermer le popup
this.activeModal.close('Data updated');
this.toastr.success("Etat général modifié avec succès!", 'STOCK-PRIX');
}else {
 this.toastr.error('Le code ou le libellé  existe déjà!', 'STOCK-PRIX');
}
      },
