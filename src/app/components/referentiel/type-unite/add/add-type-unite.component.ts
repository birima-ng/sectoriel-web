...
@Component({
  selector: 'app-add-type-unite',
  templateUrl: './add-type-unite.component.html',
  styleUrls: ['./add-type-unite.component.scss']
})

export class AddTypeUniteComponent {
invalidLogin = false;
submitted=false;
lng="en";
typeunite: TypeUnite;
...
add(){
    ...
    this.typeuniteService.createTypeUnite(this.addForm.value).subscribe(
      data => {
 if(data){
 this.spinner.hide();
...
edit(){
    ...
    this.typeuniteService.updateTypeUnite(this.addForm.value).subscribe(
      data => {
if(data){
 this.spinner.hide();
